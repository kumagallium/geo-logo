...
- **detail** — 添えの数。0（なし）または 3〜12。主役のまわりに小さな点を
  等配置します。家紋の輪郭は中央値 10 本、型だけだと 4〜8 本しかありません。
  州浜・蛇の目の鋲・七曜のように、対称を壊さず数だけ増やせる唯一の手です。
  周回・連携・多数・粒・星・時刻といった要素はここへ
...
## 点の置き方
これは**ラフのスケッチ**です。人が下描きをするのと同じで、まず形を出します。
整えるのはシステムの仕事で、あなたは形を出すことに集中してください。

- **1 つの輪郭につき 20〜45 点**。以前は 6〜14 点でしたが、それでは輪郭ではなく
  落書きになります。**円弧に均す工程が別にあるので、点を増やしても
  「なぞっただけ」にはなりません**。むしろ点が足りないと形が出ません
- **形が変わるところに置く**。直線的なところに点を足す必要はありません
- **特徴を誇張する**。ゴリラなら肩の張り、鷲なら嘴の鉤、山なら稜線の角度。
  実物の比率どおりに置くと、何の題材か分からない塊になります
- **角は角のまま残ります**。60° を超えて折れる点は角として保たれるので、
  耳の先・嘴・鰭の切れ込み・牙は、点を鋭く折り返して置いてください。
  逆に、丸めたいところは折れが 60° を超えないようになだらかに置きます
- 輪郭は 12 本まで置けます。塊が分かれる題材（翼と胴、頭と体）は分けて描き、
  重ねてください

## 抜き（hole）
白の隙間は形を読ませるために効きます。目・口・脚の間・翼の切れ込みなど、
**黒の塊を割るところ**に role: "hole" の輪郭を置いてください。

## キーライン — 重なりを分ける白
洗練された紋章ロゴは、重なった部品の境目を**一定幅の白**で分けています。
翼と胴、首と胸、顎と頬。これが無いと部品が黒い塊へ溶けます。

\`keyline\` に幅（モジュール。0.1 前後）を書けば、**後から重ねた輪郭の縁に
沿って白が回ります**。境目ごとに細長い hole を引く必要はありません。手で
引くと幅が揃わないので、重なりを分けたいときは keyline を使ってください。

輪郭は**書いた順に重なります**。奥のものから先に書いてください（奥の脚 →
胴 → 頭）。白は後から来た輪郭の下に敷かれるので、順序がそのまま前後関係に
なります。輪郭ごとに \`keyline: 0\` と書けば、その 1 本だけ溶かせます。

## 左右対称 — 迷ったら true
家紋 390 点の 80% は左右対称です。ロゴ全般もそうで、正面を向いた顔・盾・鳥は
もちろん、抽象的なマークも対称なものが多数派です。**既定は true と考え、
false にするのは向きが本質的な題材（歩く動物・横顔・流れ）だけ**にしてください。
...
## 出力
上と同じ形式の JSON。キーは name / concept / symmetry / keyline / contours。
