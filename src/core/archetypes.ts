...
export const archetypeParamsSchema = z.object({
   ...
   * 比例体系。
   *
   * 家紋 390 点で検証した結果、**半径に比例の梯子は無かった**。対数半径の
   * 自己相関を取ると √2 も φ も背景と区別が付かず、分布は連続だった
   * （scripts/kamon-stats.ts）。一度 √2 を既定にしたが、それは畳み込み
   * 閾値の副産物を読み違えたもので、戻した。
   *
   * つまりこの値は「実測に裏付けられた体系」ではなく、寸法を数種類に
   * 揃えるための道具でしかない。揃っていることに意味があり、どの比で
   * 揃えるかに意味は無い、というのが現時点で言えること。
   */
  ratio: z
    .enum(RATIOS)
    .default('golden')
